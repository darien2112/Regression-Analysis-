...
#Read in the data
power.load<-read.table(file = "C:\\Users\\pepu2\\OneDrive\\Máy tính\\Self-Studying\\STAT 450 (870) - Regression Analysis\\[Ch.3] Diagnostics and Remedial Measures\\Data\\power.txt", header=TRUE, sep = "")
...
curve(expr = mod.fit$coefficients[1] + mod.fit$coefficients[2]*x, 
      col = "red", lty = "solid", lwd = 1, add = TRUE, xlim = c(min(power.load$temp), max(power.load$temp)))
...
curve(expr = predict(object = mod.fit2, newdata = data.frame(temp = x)), 
      col = "blue", lty = "solid", lwd = 1, add = TRUE, xlim = c(min(power.load$temp), max(power.load$temp)))
